...
function initializePlayerErrorReporter(
	...
	return async (queue: GuildQueue | undefined, error: Error) => {
		...
		const embed = new EmbedBuilder()
			...
			.setDescription(
				`Could not stream ${trackInfo}. Skipping to the next track.`,
			)
			.setColor('Red')
			.setFields([
				{
		...
		try {
			...
			logger.error(skipError, 'Failed to skip errored track');
		}
		/* v8 ignore stop */
	};
}
