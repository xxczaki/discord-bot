...
> 🎵 Feature-rich Discord music bot with caching, AI-powered queue control, and more
...
The bot is designed to run as a single instance serving one Discord server at a time, typically on one voice channel at once. It does not scale horizontally – for multi-tenant hosting, run a separate instance per server.
